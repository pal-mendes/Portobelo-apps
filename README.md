...
Memória ChatGPT:
Has 3 Google Apps Script projects: AuthCoreLib library authenticates users for web apps “Associados” and “Anúncios”, running as admin@titulares-portobelo.pt (Google Workspace single user). These web apps are not embedded in Google Sites, as used to be. Associados Google Group has 308 emails (users may have 1+). Web apps require Google login and validate email against tblTitulares (Google Sheets) to locate apartments/weeks, quotas, GDPR, etc. Associados: cards per row; multi-line GDPR gate; can proceed without accepting GDPR/balance but hides parts; uses email allowlist script property (null => access to all). Anúncios: not yet using AuthCoreLib; will only allow entry with balance OK; when submitting selects phone via tblTitulares. GitHub repo Portobelo-apps structure: AuthCoreLib/src/{appsscript.json, AuthCore.js, Logger_js.html, Login.html, RGPD.html}; Associados/src/{appsscript.json, Associados.js, Main.html, Logger_js.html}; Anuncios/src/{appsscript.json, Anuncios.js, App.html, OldAuthCore.js, Form.html, OldLogin.html, Main.html, Styles.html, Logger_js.html}. When user provides a GitHub REF (commit hash), do not answer without checking on GitHub connector the code for relevant files using same hash. Prefer “cheap changes” (small diffs/patches) when possible.
